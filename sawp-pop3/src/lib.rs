...
impl POP3 {
    ...
    pub fn parse_command(input: &[u8]) -> Result<(&[u8], Message)> {
        let mut flags: Flags<ErrorFlag> = ErrorFlag::none();

        let (input, raw_keyword) = terminated(is_not(" \r"), opt(char(' ')))(input)?;
        if let Ok(keyword) = Keyword::try_from(raw_keyword) {
...
mod tests {
    ...
    #[rstest(
        ...
        case::invalid_keyword(
            ...
            Err(
                Error::from(nom::Err::Error((b"", ErrorKind::IsA))
            ))),
        ...
        case::server_response_invalid_status(
            b"+SUCCESS 2 200\r\n",
            Err(Error::from(nom::Err::Error((b"", ErrorKind::IsA))))),
    )]
